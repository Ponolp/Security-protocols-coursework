...
BASE_URL_ENCRYPT = "http://localhost:5000/dna/register"
BASE_URL_QUERY = "http://localhost:5000/analyst/query_dna"
...
def test_insert_userdata_dna(user_id, data, vector_size):
    payload = {
        'user_id': user_id,
        'data': data
    }
...
def test_query_dna(user_id, query_value, vector_size):
    payload = {
        'user_id': user_id,
...
def run_performance_tests():
    ...
    dna_data = utils.process_dna_files(DNA_DIR)
    
    query_value = generate_random_dna_sequence(2)
    ...
    total_memory_reg = 0
    total_memory_enc = 0
    for i, dataset in zip(range(user_count), dna_data):
        # Simulate the request
        elapsed_time, time_of_reg, time_of_enc, peak_memory_reg, peak_memory_enc = test_insert_userdata_dna(i, dataset, vector_size)
    ...
    for user in range(user_count):
        # Simulate the request
        elapsed_time, time_of_kd, time_of_dec, peak_memory_kd, peak_memory_dec = test_query_dna(user, query_value, vector_size)
